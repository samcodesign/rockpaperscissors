...
def chal(req):
    req.sendall(b"Beat me in Rock Paper Scissors 20 consecutive times to get the flag\n")
    for i in range(20):
        secret, rps = gen_commitments()
        move = rps[0][0]
        req.sendall(b"Here are the possible commitments, the first one is my move:" + " ".join(map(lambda s: s[1], rps)).encode() + b'\n')
        req.sendall(b"Your move: ")
        inp = req.recv(256).strip().decode()
        res = check_win(move, inp)
        req.sendall(b"My move was:" + move.encode() + b"Secret was:" + secret.encode() + b'\n')
        if not res:
            req.sendall(b"You lose!")
            exit(0)

    req.sendall(b"You win\n")
    req.sendall(b"Your reward is " + flag + b'\n')
    exit(0)
